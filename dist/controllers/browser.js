...
const ChromeLauncher = __importStar(import("chrome-launcher"));
...
const puppeteer_extra_plugin_stealth_1 = __importDefault(require("puppeteer-extra-plugin-stealth-lp"));
...
function removeStoredSingletonLock(puppeteerOptions, spinnies, options) {
    return new Promise((resolve, reject) => {
        try {
            ...
            const singletonLockPath = path.join(path.resolve(process.cwd(), userDataDir));

            fs.rmSync(path.join(singletonLockPath, 'SingletonLock'))
